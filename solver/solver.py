...
class Solver:
    ...
    def solve_instance(self) -> Tuple[KnowledgeBase, bool, List]:
        ...
        while (not solved):
            ...
            if runtime > (30 * 60):
                raise RunningTimeException(f"Skipping sudoku after runtime {runtime}")
    ...
    def inform_user(self, state : KnowledgeBase, count, start):
        ...
        if count % 25 == 0:
            runtime = timeit.default_timer() - start
            count = 1
            print(f"\rProblem: {self.problem_id} Length solution: {len(state.current_set_literals)} out of"
